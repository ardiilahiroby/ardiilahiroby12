...
<head>
    <meta charset="UTF-8">
    <title>Galeri | Ardi Ilahi Roby</title>
...
<body class="bg-white dark:bg-gray-900 text-black dark:text-white transition-colors duration-300">
    ...
    <nav class="bg-gray-200 dark:bg-gray-800 shadow">
        <div class="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <div class="flex space-x-4 items-center">
                ...
                <a href="gallery.php" class="font-bold underline text-blue-600 dark:text-blue-400">Galeri</a>
    ...
    <main class="max-w-6xl mx-auto p-6 mt-8">
        <h2 class="text-3xl font-bold text-center mb-6 text-blue-700 dark:text-blue-400">Galeri Foto</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            <?php
            $sql = "SELECT * FROM tbl_gallery ORDER BY id_gallery DESC";
            $query = mysqli_query($db, $sql);
            while ($data = mysqli_fetch_array($query)) {
                $src = 'images/' . htmlspecialchars($data['foto']);
                $judul = htmlspecialchars($data['judul']);
                echo "
                <div class='bg-gray-100 dark:bg-gray-800 p-4 rounded-xl shadow cursor-pointer'>
                    <img src='$src' alt='$judul' class='w-full h-64 object-cover rounded-lg' onclick=\"openModal('$src', '$judul')\">
                    <h3 class='mt-2 text-blue-700 dark:text-blue-400 font-semibold'>$judul</h3>
                </div>";
            }
            ?>
        </div>
