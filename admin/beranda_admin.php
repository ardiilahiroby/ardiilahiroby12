...
$jumlah_galeri = mysqli_num_rows(mysqli_query($db, "SELECT id_gallery FROM tbl_gallery"));
$jumlah_kontak = mysqli_num_rows(mysqli_query($db, "SELECT id_contact FROM tbl_contact"));
?>
<!DOCTYPE html>
<html lang="id">
...
<body class="bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200">
    ...
    <nav class="bg-white dark:bg-gray-800 shadow p-4 flex justify-between items-center">
        <h1 class="text-xl font-bold text-blue-700 dark:text-blue-300">Dashboard Admin</h1>
        <ul class="flex space-x-4 items-center">
            ...
            <li>
                <a href="logout.php" onclick="return confirm('Apakah anda yakin ingin keluar?');"
                    class="text-red-600 hover:underline font-medium">Logout</a>
            </li>
            <li>
                <button onclick="toggleDarkMode()" class="bg-gray-300 dark:bg-gray-600 text-sm px-2 py-1 rounded">
                    🌙
                </button>
            </li>
        </ul>
    ...
    <main class="flex-1 p-6">
        <h2 class="text-2xl font-semibold mb-6">Selamat datang, <?= $admin['username'] ?></h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                <h3 class="text-lg font-semibold mb-2">Total Artikel</h3>
                <p class="text-3xl"><?= $jumlah_artikel ?></p>
            </div>
            <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                <h3 class="text-lg font-semibold mb-2">Total Galeri</h3>
                <p class="text-3xl"><?= $jumlah_galeri ?></p>
            </div>
            <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                <h3 class="text-lg font-semibold mb-2">Kontak Masuk</h3>
                <p class="text-3xl"><?= $jumlah_kontak ?></p>
            </div>
        </div>
    </main>
