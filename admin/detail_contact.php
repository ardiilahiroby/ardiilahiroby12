...
$result = mysqli_query($db, "SELECT * FROM tbl_contact WHERE id_contact = $id");
...
<body class="bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200">
    <div class="max-w-xl mx-auto mt-12 p-6 bg-white dark:bg-gray-800 rounded shadow">
        <h2 class="text-xl font-bold text-center text-blue-600 mb-4">Detail Pesan</h2>
        <p><strong>Nama:</strong> <?= htmlspecialchars($data['nama']) ?></p>
        <p><strong>Email:</strong> <?= htmlspecialchars($data['email']) ?></p>
        <p><strong>Tanggal:</strong> <?= htmlspecialchars($data['tanggal_kirim']) ?></p>
        <p class="mt-4"><strong>Pesan:</strong></p>
        <div class="p-3 border rounded bg-gray-50 dark:bg-gray-700 mt-2"><?= nl2br(htmlspecialchars($data['pesan'])) ?>
        </div>
        <div class="mt-6 text-center">
            <a href="data_contact.php" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">← Kembali</a>
