...
$sql = "SELECT * FROM tbl_contact WHERE nama LIKE '%$keyword%' OR email LIKE '%$keyword%' ORDER BY tanggal_kirim DESC";
...
<body class="bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200">
    ...
    <nav class="bg-white dark:bg-gray-800 shadow p-4 flex justify-between items-center">
        <h1 class="text-xl font-bold text-blue-700 dark:text-blue-300">KELOLA KONTAK</h1>
        <ul class="flex space-x-4 items-center">
            <a href="beranda_admin.php" class="hover:underline">Beranda</a>
            <a href="about.php" class="hover:underline">Tentang</a>
            <a href="data_artikel.php" class="hover:underline">Artikel</a>
            <a href="data_gallery.php" class="hover:underline">Gallery</a>
            <a href="data_contact.php" class="hover:underline">Kontak</a>
            <a href="logout.php" onclick="return confirm('Apakah Anda yakin ingin keluar?');"
                class="text-red-300 hover:text-white font-medium">Logout</a>
            <!-- Tombol dark mode -->
    ...
    <div class="max-w-6xl mx-auto mt-10 p-6 bg-white dark:bg-gray-800 rounded shadow">
        <h2 class="text-2xl font-bold mb-6 text-center text-blue-600 dark:text-blue-300">Pesan Masuk</h2>

        <!-- Form Pencarian -->
        <form method="GET" class="mb-4 text-center">
            <input type="text" name="cari" value="<?= htmlspecialchars($keyword) ?>"
                placeholder="Cari nama atau email..."
                class="p-2 rounded border dark:bg-gray-700 dark:border-gray-600 text-sm w-1/2">
            <button type="submit"
                class="bg-blue-600 text-white px-4 py-1.5 rounded hover:bg-blue-700 text-sm">Cari</button>
        </form>
                ...
                <tbody class="text-gray-800 dark:text-gray-100">
                    ...
                    if (mysqli_num_rows($query) > 0) {
                        while ($data = mysqli_fetch_assoc($query)) {
                            ...
                            echo "<td class='border p-3'>" . htmlspecialchars($data['nama']) . "</td>";
                            echo "<td class='border p-3'>" . htmlspecialchars($data['email']) . "</td>";
                            echo "<td class='border p-3'>" . htmlspecialchars($data['tanggal_kirim']) . "</td>";
                            echo "<td class='border p-3 space-x-2 text-center'>
                                <a href='detail_contact.php?id={$data['id_contact']}' class='bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded'>Lihat</a>
                                <a href='hapus_contact.php?id={$data['id_contact']}' onclick='return confirm(\"Yakin hapus pesan ini?\")' class='bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded'>Hapus</a>
                              </td>";
                            echo "</tr>";
                            $no++;
                        }
                    } else {
                        echo "<tr><td colspan='5' class='text-center p-4 text-gray-500 dark:text-gray-400'>Tidak ada data ditemukan.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
        ...
        <div class="mt-6 text-center">
            <a href="beranda_admin.php" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded text-sm">←
    ...
    <script>
        function toggleDarkMode() {
            ...
            // Simpan preferensi di localStorage agar bertahan saat reload
        ...
        // Cek preferensi saat halaman pertama kali dimuat
